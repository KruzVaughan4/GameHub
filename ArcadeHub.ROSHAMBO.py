...
def save_high_score():
    global hiWinCount, hiLostCount, hiTieCount
    if (game.winCount > hiWinCount or
        (game.winCount == hiWinCount and game.lostCount < hiLostCount) or
        (game.winCount == hiWinCount and game.lostCount == hiLostCount and game.tieCount < hiTieCount)):
        with open("hiscore.txt", "w") as hiscoreFile:
            hiscoreFile.write(f"{game.winCount}\n{game.lostCount}\n{game.tieCount}\n")
...
try:
    with open("hiscore.txt", "r") as hiscoreFile:
        hiWinCount = int(hiscoreFile.readline().strip() or 0)
        hiLostCount = int(hiscoreFile.readline().strip() or 0)
        hiTieCount = int(hiscoreFile.readline().strip() or 0)
except FileNotFoundError:
    hiWinCount = hiLostCount = hiTieCount = 0
...
while running:
    ...
    screen.blit(font.render(f"High Score: {hiWinCount} - {hiLostCount} - {hiTieCount}", True, WHITE),
                (WIDTH - 310, HEIGHT - 40))
